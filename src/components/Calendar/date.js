...
Calendar.prototype = {
  ...
  format(str) {
    const separators = str.replace && str.replace(/(YYYY)|MM|DD/g, '').split('')
  ...
  nextMonth(month) {
    const nextMonthDays = month + 1
    const isNotNextYear = nextMonthDays < this.mouths.length
    const index = isNotNextYear ? nextMonthDays : 0
    const days = new Array(this.mouths[index] + 1).join('*').split('').map((o, i) => i + 1)
    return {
  ...
  prevMonth(month) {
    ...
    const isPrevYear = prevMonthDays < 0
    const index = isPrevYear ? 11 : prevMonthDays
    const days = new Array(this.mouths[index] + 1).join('*').split('').map((o, i) => i + 1)
    return {
      isPrevYear,
      days,
  ...
  paint(month, year) {
    ...
    this.leap()
    const currentDays = this.mouths[this.currentMonth] + 1
    const paint = this.getCalendar(currentDays, this.currentYear, this.currentMonth)
    this.options.callback && this.options.callback(paint)
    return this
  },
  next() {
    const isNotNextYear = this.currentMonth + 1 < this.mouths.length
    this.currentMonth = isNotNextYear ? this.currentMonth + 1 : 0
    if (!isNotNextYear) this.currentYear = this.currentYear + 1
    return this
  },
  prev() {
    const isPrevYear = this.currentMonth - 1 < 0
  ...
  getCalendar(currentDays, year, month) {
    const posDay = this.getPosDay(year, month)
    const otherMonth = this.getOtherMonth(this.currentMonth)
    const nextPosDay = this.getPosDay(year, month, currentDays - 1)
    const prevMonthDays = posDay !== 0 ? otherMonth.prevMonth.days.slice(-posDay) : []
    const nextMonthDays = otherMonth.nextMonthDays.days.slice(0, 6 - nextPosDay)

    const prevYear = otherMonth.prevMonth.isPrevYear ? year - 1 : year
    const nextYear = otherMonth.nextMonthDays.isNextYear ? year + 1 : year
